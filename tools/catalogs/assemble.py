...
BR=['caprari','dreno','masdaf','tsunami','purity','aikon','aquastrong','vandjord','fancy','kq','homa','liancheng','sempa','standart','shinmaywa','hydroo','shimge','aquario','speroni','jung','pentax','herborner']
...
log['aquastrong']+=[tuple(x) for x in json.load(open('out/aquastrong_excluded.json'))]
log['homa']+=[tuple(x) for x in json.load(open('out/homa_excluded.json'))]
...
log['shimge']+=[tuple(x) for x in json.load(open('out/shimge_excluded.json'))]
log['speroni']+=[tuple(x) for x in json.load(open('out/speroni_excluded.json'))]
log['jung']+=[tuple(x) for x in json.load(open('out/jung_excluded.json'))]
